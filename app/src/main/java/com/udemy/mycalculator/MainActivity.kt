...
class MainActivity : AppCompatActivity() {
    ...
    //Check if this is where this goes...remember last app
    ...
    // Function for button inputs
    fun onDigit(view : View){
        // Appends text from button. "view as Button" is used to take the value of the button as the text
        tvInput?.append((view as Button).text)
        lastNumeric = true //This sets the last numeric to true to the loop in the onDecimal can function
        lastDot = false
    ...
    fun onDecimalPoint(view : View){
        // If last numeric value is false and the last value is not a dot
        // onclick must set lastNumeric to true for for loop to iterate
        // If the last appended value is numeric then append(".") or don't.
        if (lastNumeric && !lastDot){
            tvInput?.append(".")
            lastNumeric = false // If this code is ran, last numeric remains false
            lastDot = true // since last value is now a dot, it sets the boolean to true
    ...
    fun onOperator(view : View){
        tvInput?.text?.let{
            ...
            // Will execute if tvInput and text are not null
            if(lastNumeric && !isOperatorAdded(it.toString())){ //if last entry was numeric & is isOperator is false
                tvInput?.append((view as Button).text)
    ...
    fun onEqual(view: View){
       ...
       if (lastNumeric){
           //Converts tvInput from text to a string if lastNumeric is true
